...
function StartQuizIntro({ onStart }) {
  return (
    <section
      ...
      className="relative overflow-hidden rounded-xl border border-border-glass bg-surface-solid p-6 text-center backdrop-blur-sm sm:p-8"
    >
      ...
      <h2 id="start-quiz-title" className="mt-3 font-display text-3xl font-bold tracking-tight text-copy">
        Start the Rick and Morty Quiz
      </h2>
      <p id="start-quiz-description" className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-copy-muted">
      ...
      <div className="mx-auto mt-6 grid max-w-lg grid-cols-3 gap-3 text-center">
        <div className="rounded-lg border border-border-glass bg-surface-glass px-3 py-3">
          ...
          <div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-copy-faint">Questions</div>
        </div>
        <div className="rounded-lg border border-border-glass bg-surface-glass px-3 py-3">
          <div className="font-display text-xl font-bold text-electric-blue">{quizQuestions.length}</div>
          <div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-copy-faint">In Bank</div>
        </div>
        <div className="rounded-lg border border-border-glass bg-surface-glass px-3 py-3">
          <div className="font-display text-xl font-bold text-accent-amber">1</div>
          <div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-copy-faint">Attempt</div>
        </div>
      </div>
      <button
        type="button"
    ...
    return (
      <div className="relative mx-auto min-h-[calc(100vh-9rem)] max-w-5xl px-4 py-10 sm:py-14">
        <section className="mb-8">
          ...
          <h1 className="mt-3 font-display text-3xl font-bold tracking-tight text-copy sm:text-4xl" style={{ textWrap: 'balance' }}>
            Test your multiverse recall
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-copy-muted sm:text-base">
    ...
    return (
      <div className="mx-auto max-w-5xl px-4 py-10 sm:py-14">
        <section className="relative overflow-hidden rounded-xl border border-border-glass bg-surface-glass px-5 py-8 text-center backdrop-blur-sm sm:px-8 sm:py-10">
          ...
          <h1 className="mt-3 font-display text-4xl font-bold tracking-tight text-copy sm:text-5xl">
            {score}/{QUIZ_SIZE}
          </h1>
          <p className="mt-2 text-lg font-semibold text-electric-blue">{percentage}% correct</p>
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-copy-muted">
            {getResultCopy(score)}
          </p>
          <button
            type="button"
        ...
        <section className="mt-8" aria-labelledby="quiz-review-title">
          <h2 id="quiz-review-title" className="font-display text-2xl font-bold tracking-tight text-copy">
            Answer Review
          </h2>
              ...
              return (
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <h3 className="font-display text-lg font-semibold text-copy">
                  ...
                  <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                    <div>
                      <dt className="text-copy-faint">Your answer</dt>
                      <dd className={isCorrect ? 'mt-1 text-portal-green' : 'mt-1 text-dead'}>
                        {selectedKey}. {getOptionLabel(question, selectedKey)}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-copy-faint">Correct answer</dt>
                      <dd className="mt-1 text-copy-soft">
      ...
      >
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-copy-faint">{answeredCount} of {QUIZ_SIZE} answered</p>
          </div>
          <button
            type="button"
            onClick={handleStartQuiz}
            className="w-fit rounded-lg border border-border-glass bg-surface-raised px-4 py-2 text-sm text-copy-soft transition-[border-color,background-color,color] hover:border-electric-blue/40 hover:bg-surface-hover hover:text-electric-blue focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric-blue"
          >
            New Random Set
          </button>
        </div>

        <div className="mt-5 h-2 overflow-hidden rounded-full bg-surface-solid">
          <div
        ...
        <fieldset className="mt-8">
          <legend id="quiz-question-title" className="font-display text-2xl font-bold leading-tight text-copy" style={{ textWrap: 'balance' }}>
            ...
            {currentQuestion.options.map(option => {
              ...
              return (
                <label
                  key={option.key}
                  htmlFor={inputId}
                  className={`group flex min-h-16 cursor-pointer items-center gap-4 rounded-lg border px-4 py-3 transition-[border-color,background-color,box-shadow] ${isSelected ? 'border-portal-green/60 bg-portal-green/10 shadow-[0_0_22px_rgba(57,231,95,0.1)]' : 'border-border-glass bg-surface-solid hover:border-electric-blue/40 hover:bg-surface-raised'}`}
                >
                  <input
                    id={inputId}
                    type="radio"
                  ...
                  <span className="text-sm leading-relaxed text-copy-soft sm:text-base">{option.label}</span>
                </label>
              )
            })}
          </div>
        ...
        <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <button
            ...
            className="rounded-lg border border-border-glass bg-surface-raised px-5 py-3 font-display text-sm font-semibold text-copy-soft transition-[border-color,background-color,color,opacity] hover:border-electric-blue/40 hover:bg-surface-hover hover:text-electric-blue focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric-blue disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-border-glass disabled:hover:bg-surface-raised disabled:hover:text-copy-soft"
          >
            Back
          </button>
